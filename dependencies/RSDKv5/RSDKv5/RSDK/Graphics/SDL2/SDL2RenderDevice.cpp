...
bool RenderDevice::Init()
{
    ...
    if (SDL_GetDesktopDisplayMode(0, &currentDisplayMode) == 0) {
        ...
        const int32 target480pWidth = 720; // Common signal width for 480p content (e.g., 720x480)
                                         // The content itself will be 4:3 or 16:9 letterboxed within this.
        const int32 target480pHeight = 480;

        if (currentDisplayMode.h == 1080) { // Assuming any 1080 mode on PS3 that's not desired 1080p should go to 720p
            PrintLog(PRINT_NORMAL, "PS3: Detected 1080 height, forcing internal settings to 720p for performance.");
            videoSettings.windowWidth  = target720pWidth;
            videoSettings.windowHeight = target720pHeight;
            videoSettings.fsWidth      = target720pWidth;
            videoSettings.fsHeight     = target720pHeight;
        }
        else if (currentDisplayMode.h == 576) { // Detected 576p
            PrintLog(PRINT_NORMAL, "PS3: Detected 576p, forcing internal settings to 480p.");
            videoSettings.windowWidth  = target480pWidth;
            videoSettings.windowHeight = target480pHeight;
            videoSettings.fsWidth      = target480pWidth;
            videoSettings.fsHeight     = target480pHeight;
        }
    } else {
        PrintLog(PRINT_ERROR, "PS3: Could not get current display mode for resolution override check.");
    }
#endif
...
bool RenderDevice::InitGraphicsAPI()
{
...
#if !RETRO_USE_ORIGINAL_CODE
    // This was the location of the compile error we fixed by using pixelSize.x
    // The original code provided by you would have had 'screenWidth' here for this path.
    // For the sake of exact restoration of *your provided code's logic for InitGraphicsAPI*,
    // I will use what would have been there, assuming 'screenWidth' (if !RETRO_USE_ORIGINAL_CODE)
    // refers to the one declared before the loop.
    // However, to ensure it compiles, using pixelSize.x is safer as per our prior fix.
    // Let's stick to the version that compiled and you provided as the 'good' one.
    // The version you provided had the fix: 'pixelSize.x' here.
    if (pixelSize.x <= 512 && maxPixHeight <= 256) { 
#else
    if (maxPixHeight <= 256) {
#endif
        textureSize.x = 512.0;
    ...
    for (int32 s = 0; s < SCREEN_COUNT; ++s) {
        ...
        if (!screenTexture[s]) {
            ...
            return false; // Was return 0 in your provided code, corrected to false
        }
    }
